...
# @dataclass
# class OnlyAnswerLossTemplate(Template):
#     r"""Only compute loss inside the <answer>...</answer> region."""
...
#         tools: Optional[str],
#     ) -> list[list[int]]:
#         r"""Only keep labels for <answer>...</answer> in assistant messages"""
#         system = system or self.default_system
#         encoded_messages = []
...
#             # 只对 assistant 的内容做 loss masking
#             if message["role"] == Role.ASSISTANT.value:
#                 decoded_text = tokenizer.decode(token_ids, skip_special_tokens=True)
#                 answer_start = decoded_text.find("<answer>")
#                 answer_end = decoded_text.find("</answer>") + len("</answer>")

#                 if answer_start == -1 or answer_end == -1:
#                     labels = [-100] * len(token_ids)
#                 else:
#                     prefix_tokens = tokenizer(decoded_text[:answer_start], add_special_tokens=False).input_ids
#                     answer_tokens = tokenizer(decoded_text[answer_start:answer_end], add_special_tokens=False).input_ids

#                     labels = [-100] * len(token_ids)
#                     labels[len(prefix_tokens):len(prefix_tokens) + len(answer_tokens)] = token_ids[len(prefix_tokens):len(prefix_tokens) + len(answer_tokens)]

#                 encoded_messages.append(labels)
#             else:
...
#         """构造一个样本，返回 input_ids 和 masked labels（只在 <answer> 计算 loss）"""
#         prompt_ids, response_ids = self.encode_oneturn(
...
#         # 获取最后一轮 assistant 回复内容（包含 <think><answer>）
#         response_text = example["conversations"][-1]["content"]
#         answer_start = response_text.find("<answer>")
#         answer_end = response_text.find("</answer>") + len("</answer>")

#         if answer_start == -1 or answer_end == -1:
#             # 如果没找到 <answer> 区域，则整段 response 都不训练
#             labels[len(prompt_ids):] = [-100] * len(response_ids)
#         else:
#             before_answer_text = response_text[:answer_start]
#             answer_text = response_text[answer_start:answer_end]

#             before_answer_ids = self.tokenizer(before_answer_text, add_special_tokens=False).input_ids
#             answer_ids = self.tokenizer(answer_text, add_special_tokens=False).input_ids

#             # 将非 answer 区域 label 置为 -100
#             for i in range(len(response_ids)):
#                 if i < len(before_answer_ids) or i >= len(before_answer_ids) + len(answer_ids):
#                     labels[len(prompt_ids) + i] = -100
