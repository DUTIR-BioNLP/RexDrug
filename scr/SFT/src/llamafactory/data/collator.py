...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        for feature in features:
            ...
            # batch_loss_weight.append(feature.pop("loss_weight"))
        ...
        # if "loss_weight" not in features:           # super() 不会自动 pad 这个字段
        #     max_len = features["input_ids"].size(1)  # batch 序列最大长度
        #     lw_tensor = torch.zeros(
        #         (len(batch_loss_weight), max_len), dtype=torch.float32
        #     )
        #     for i, lw in enumerate(batch_loss_weight):
        #         lw_tensor[i, : len(lw)] = torch.tensor(lw, dtype=torch.float32)

        #     features["loss_weight"] = lw_tensor
