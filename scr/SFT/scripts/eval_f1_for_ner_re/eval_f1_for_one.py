...
parser.add_argument('--data-file', type=str, required=False, default="third_4080.jsonl", help="Path to the data file")
parser.add_argument('--output', type=str, required=False, default="eval_F1.jsonl", help="Path to the output metrics file")
...
def create_vectors(gold: List[Dict[str, Any]], test: List[Dict[str, Any]], exact_match: bool, any_comb: bool) \
    ...
    for rel1 in gold:
        ...
        for k, rel2 in enumerate(test):
            ...
            if ((spans_intersecting >= 2) and (not exact_match)) or (score >= 0.999999999999999999):
...
if __name__ == "__main__":
    ...
    for entry in data:
        entry['predict'] = extract_re(entry['predict'])
        entry['label'] = extract_re(entry['label'])
    ...
    with open(args.output, "a+") as f_out:
        json.dump(
            {
                "The_target_experiment": "llama3_1_rank128_18375.json",
